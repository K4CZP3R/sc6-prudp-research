...
export class Packet implements IPacket {
    ...
    static fromHeaderAndBuffer(header: IPacketHeader, rawData: Buffer) {
        ...
        let potentialPacket = new Packet(getPacketLocation(header.source), getPacketLocation(header.destination), type, getPacketFlags(header.type_and_flags), header.session_id, header.packet_signature, header.sequence_id, packet_specific_data, payload, checksum);

        // Check if rawData is equal to the buffer generated by toBuffer()
        let generatedBuf = potentialPacket.toBuffer();
        console.log("Debug", {
            generatedBuf: generatedBuf.toString("hex"),
            rawData: rawData.toString("hex"),
            genDescribe: packetDescribe(potentialPacket)
        })
    ...
    toBuffer(): Buffer {
        ...
        let locationBuf = Buffer.from([fromPacketLocation(this.source), fromPacketLocation(this.destination)]);
        ...
        console.log("ready", ready.toString("hex"));
        return ready;
...
function getPacketLocation(location: number): PacketLocation {
    switch (location) {
        case 0x31:
            return "server";
        case 0x3f:
...
export function fromPacketLocation(packetLocation: PacketLocation): number {
    switch (packetLocation) {
        case "server":
            return 0x31;
        case "client":
            return 0x3f;
        default:
            throw new Error("Invalid packet location");
    }
}
